...
const App: React.FC = () => {
  ...
  // FacilitatorDashboard → QuizStage 로 넘어올 때 카테고리 전달
  ...
  useEffect(() => {
    const handleNavigation = (e: CustomEvent) => {
      ...
      if (typeof detail === 'string') {
        setCurrentView(detail as ViewName);
      }
      // 객체이면 view + 추가 데이터
      else if (detail && typeof detail === 'object') {
        setCurrentView(detail.view as ViewName);
  ...
  return (
    <div className="min-h-screen bg-[#020205] text-slate-200 font-sans transition-all duration-300 select-none">
      ...
      <main className="relative w-full overflow-x-hidden pt-24 min-h-screen">
        ...
        {currentView === 'quiz' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <QuizStage initialCategories={quizCategories} initialCode={quizCode} />
          </div>
        )}
