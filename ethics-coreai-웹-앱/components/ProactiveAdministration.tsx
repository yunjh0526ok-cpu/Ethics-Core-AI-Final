...
<style>{@keyframes marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } } .animate-marquee { display: flex; width: max-content; animation: marquee 40s linear infinite; } .animate-marquee:hover { animation-play-state: paused; }}</style>
  ...
  <nav className="fixed top-0 left-0 right-0 z-50 bg-[#050A15]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex justify-between items-center">
    ...
    <div className="hidden md:flex items-center gap-8 text-xs font-bold text-slate-400 uppercase tracking-widest">
      ...
      <span>About Center</span><span>AI Proposal</span><span>Fun & Play</span><span>AI Solutions</span><span>Core Services</span><span>Portfolio</span>
    </div>
  ...
  <main className="pt-32 pb-20 px-6 max-w-[1400px] mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      {/* 4. 좌측 대시보드 복구 (오늘의 상담, 해결률, 실시간 이슈) */}
      <div className="lg:col-span-3 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="p-6 rounded-3xl bg-[#0D1425] border border-white/5 shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase mb-1 tracking-tighter tracking-widest">Today's Consultations</p>
          ...
          <div className="p-6 rounded-3xl bg-[#0D1425] border border-white/5 shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase mb-1 tracking-widest">Solution Rate</p>
          ...
          <div className="space-y-6">
            {[ { tag: '적극행정 면책', val: 85 }, { tag: '사전컨설팅', val: 72 }, { tag: '2025 우수사례', val: 68 }, { tag: '주양순 강사', val: 55 } ].map((item, idx) => (
              <div key={idx} className="space-y-2">
                ...
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden"><div style={{ width: `${item.val}%` }} className="h-full bg-blue-500" /></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#0D1425] rounded-[3rem] border border-white/10 shadow-3xl flex flex-col h-full overflow-hidden relative">
          ...
          <div className="px-8 py-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div><h3 className="text-md font-black text-white">상담관 든든이</h3><p className="text-[9px] text-blue-500 font-bold uppercase tracking-widest">Proactive Admin AI Partner</p></div>
            </div>
            <div className="flex gap-2">
              <button className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 hover:text-white transition-all">법령 기준</button>
              <button className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 hover:text-white transition-all">면책 지원</button>
            </div>
          </div>
          ...
          <div className="p-8 bg-white/[0.01] border-t border-white/5 space-y-4">
            <div className="flex flex-wrap gap-2">
               {["적극행정 면책 요건 확인", "2026년 전문강사단 자격", "사전컨설팅 신청 절차"].map(q => (
                 <button key={q} onClick={() => handleSend(q)} className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold text-slate-500 hover:text-white hover:border-blue-500/50 transition-all">Q. {q}</button>
               ))}
            </div>
    ...
    <div className="mt-12 flex justify-center gap-6">
      <button onClick={goToCorruption} className="flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-[#0D1425] border border-white/10 text-slate-400 hover:text-red-400 transition-all group">
      ...
      <button onClick={goToRecovery} className="flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-[#0D1425] border border-white/10 text-slate-400 hover:text-emerald-400 transition-all group">
