      ...
      <main className="pt-32 pb-20 px-6 max-w-[1400px] mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-400 max-w-2xl mx-auto text-lg font-medium opacity-90">
            ...
            <span className="text-white font-bold underline decoration-blue-500">주양순 전문강사</span>의 솔루션으로 실시간 상담하세요.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-3 space-y-6">
            <div className="p-8 rounded-[2.5rem] bg-[#0D1425] border border-white/5 shadow-2xl">
              ...
              <div className="space-y-6 text-xs font-bold text-slate-500">
                <div className="space-y-2"><span>#면책 제도 85%</span><div className="h-1.5 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[85%]" /></div></div>
                <div className="space-y-2"><span>#우수사례 72%</span><div className="h-1.5 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[72%]" /></div></div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-[#0D1425] rounded-[3rem] border border-white/10 shadow-3xl flex flex-col h-full overflow-hidden relative">
              <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} items-end gap-3`}>
                    <div className={`max-w-[80%] p-6 rounded-[2rem] text-[15px] leading-relaxed shadow-xl ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none shadow-blue-900/20' : 'bg-[#161D2F] text-slate-200 rounded-bl-none border border-white/5'}`}>
                      {msg.text}
                    </div>
                  </div>
                ))}
              ...
              <div className="p-8 bg-white/[0.01] border-t border-white/5 space-y-4">
                <div className="flex items-center gap-4">
                  <input value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="궁금한 내용을 입력하세요..." className="flex-1 bg-[#161D2F] border border-white/10 rounded-2xl px-8 py-5 text-white focus:outline-none" />
                ...
                {showBridge && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-[#050A15]/95 backdrop-blur-xl rounded-[3rem] flex items-center justify-center p-8">
                    <div className="max-w-md w-full text-center">
                      ...
                      <p className="mb-10 text-slate-400 text-sm p-6 bg-white/5 rounded-3xl border border-white/5"><span className="text-white font-bold underline decoration-blue-500">주양순 전문강사</span>의 최신 지식 데이터가 탑재된 보안 상담 모드로 이동합니다.</p>
        ...
        <div className="mt-12 flex justify-center gap-6">
          <button onClick={goToCorruption} className="flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-[#0D1425] border border-white/10 text-slate-400 hover:text-red-400 transition-all shadow-xl"><AlertTriangle className="w-6 h-6 text-red-600" /> 부패상담관 이동</button>
          <button onClick={goToRecovery} className="flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-[#0D1425] border border-white/10 text-slate-400 hover:text-emerald-400 transition-all shadow-xl"><Coins className="w-6 h-6 text-emerald-600" /> 환수법 상담소 이동</button>
        </div>
      </main>
    </div>
  );
