...
const CuteRobot: React.FC<CuteRobotProps> = ({ view }) => {
  ...
  // Define target position based on view
  // Home (Main) & Split Views -> Center (x=0)
  // Others (Single Box) -> Right Peek (x=4)
  const targetX = (view === 'home' || !view || view === 'proposal' || view === 'about' || view === 'integrity') ? 0 : 4;
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
        ...
        // Adjust base rotation based on view if needed, but 0 is good for center.
        const baseRotation = (view === 'home' || !view || view === 'proposal' || view === 'about' || view === 'integrity') ? 0 : -0.4;
  ...
  return (
    // Initial position starts at x=4, y=0. Position is controlled by useFrame logic above.
