...
import { Bot, ChevronRight, ShieldCheck, ArrowRight, MessageCircle, Box, Home, Sparkles, ArrowLeft, Handshake, FileText, UserCheck, GraduationCap, HelpCircle, ExternalLink } from 'lucide-react';
...
const Hero: React.FC = () => {
  ...
  const partnerItems = [
      { label: "강의 의뢰 및 사업 협업", icon: Handshake, action: 'contact' },
      { label: "청렴 시민 감사관 및 정책 자문 문의", icon: UserCheck, action: 'contact' },
      { 
          label: "AI 기반 청렴·인권교육 미래대학 프로그램", 
          icon: GraduationCap, 
          action: 'https://blog.naver.com/yszoo1467/224180090553',
          isExternal: true 
      },
      { label: "기타 문의", icon: HelpCircle, action: 'contact' },
  ];
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-cyber-purple p-8 flex items-center gap-5 relative overflow-hidden shrink-0 h-[140px]">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-blue-100 text-sm opacity-90">Ethics-Core AI Agent</p>
                            </div>
                        </div>
                    </div>

                    {/* Center Content */}
                    <div className="flex-grow flex flex-col items-center justify-start relative z-10 p-6 bg-gradient-to-b from-slate-900 to-[#020205] overflow-hidden">
                         {/* Background Grid Pattern - Local overlay for box texture */}
                        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:20px_20px] pointer-events-none" />

                        {/* Icon Container - Reduced size for better fit */}
                        <motion.div 
                          className="w-16 h-16 mb-4 rounded-full bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-600/50 flex items-center justify-center shadow-[0_0_50px_rgba(59,130,246,0.15)] group-hover:scale-105 transition-transform duration-500 relative z-10 shrink-0"
                        >
                             <Sparkles className="w-8 h-8 text-cyber-400" />
                        ...
                        <div className="relative z-10 w-full flex flex-col h-full overflow-hidden">
                          <div className="shrink-0 text-center mb-4">
                              <h3 className="text-xl font-black text-white leading-tight mb-1 tracking-tight font-tech">
                                Ethics-CoreAI Partner
                              </h3>
                              <p className="text-sm text-slate-300 font-medium leading-relaxed break-keep">
                                당신의 <span className="text-white font-bold">청렴 파트너</span>가 되어 드립니다.
                              </p>
                          </div>

                          {/* PARTNER LIST BUTTONS - Scrollable */}
                          <div className="space-y-2 w-full overflow-y-auto custom-scrollbar pr-1 pb-2">
                              {partnerItems.map((item, idx) => (
                                  <button 
                                    key={idx}
                                    onClick={() => item.isExternal ? window.open(item.action, '_blank') : handleMenuClick(item.action)} 
                                    className={`w-full py-3 px-4 bg-slate-800/50 hover:bg-cyber-900/50 border border-slate-700 hover:border-cyber-500 rounded-xl flex items-center justify-between group/item transition-all ${item.isExternal ? 'hover:shadow-[0_0_15px_rgba(255,110,30,0.2)]' : ''}`}
                                  >
                                      <div className="flex items-center gap-3 overflow-hidden">
                                          <div className={`p-1.5 bg-slate-900 rounded-lg shrink-0 transition-colors ${item.isExternal ? 'text-[#ff6e1e] group-hover/item:text-white' : 'text-cyber-400 group-hover/item:text-white'}`}>
                                              <item.icon className="w-4 h-4" />
                                          </div>
                                          <span className={`text-slate-200 font-bold text-xs sm:text-sm text-left truncate leading-tight ${item.isExternal ? 'group-hover/item:text-[#ff6e1e]' : 'group-hover/item:text-white'}`}>
                                              {item.label}
                                          </span>
                                      </div>
                                      {item.isExternal ? (
                                         <div className="flex items-center gap-1 text-[10px] text-[#ff6e1e] font-bold bg-[#ff6e1e]/10 px-2 py-1 rounded border border-[#ff6e1e]/20 shrink-0">
                                            상세 <ExternalLink className="w-3 h-3" />
                                         </div>
                                      ) : (
                                         <ChevronRight className="w-4 h-4 text-slate-600 group-hover/item:text-white shrink-0" />
                                      )}
                                  </button>
                              ))}
                          </div>
